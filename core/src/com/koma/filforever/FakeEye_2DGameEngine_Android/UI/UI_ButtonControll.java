...
public class UI_ButtonControll extends UI_IMenuControll {
    public String Text;
    ...
    public UI_ButtonControll(Vector2 position, Color buttonColor, Color textColor, String text, Vector2 size, String fontName, int layer) {
        super(position, size);

        HoverController = new FloatController(5.0f);

        ControllPosition = position.sub(size.div(2));
        ControllSize = size;
    ...
    public void Draw(SpriteAdapter spriteBatch, Vector2 Offset, int layer) {
        Vector2 TSize = Size.mul(1 + HoverController.GetRegularValue() * 0.1f);

        spriteBatch.FillRectangle(Position.sub(TSize.div(2)).add(Offset), TSize, Color.Black, 1, 0, layer);
        spriteBatch.FillRectangle(Position.sub(TSize.div(2)).add(Offset).add(new Vector2(2)), TSize.sub(new Vector2(4)), ButtonColor, 1, 0, layer + 1);

        DrawIco(spriteBatch, Position.add(Offset), TSize, layer);
        DrawText(spriteBatch, Position.add(Offset), 1 + (HoverController.GetRegularValue() * 0.1f), layer);
    }
